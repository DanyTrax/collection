...
<head>
    <meta charset="UTF-8">
    <title>Cuenta <?= htmlspecialchars($cuenta->numeroCuenta) ?></title>
    <style>
    ...
    <div id="pdf-document">
        <div class="header">
            ...
            <div class="header-col" style="text-align:right;">
                <p class="uppercase muted">Cuenta de Cobro</p>
                <p class="title-number">Nº <?= htmlspecialchars($cuenta->numeroCuenta) ?></p>
                <dl>
                    <div><dt>Ciudad y fecha:</dt><dd><?= htmlspecialchars($datosEmisor['Ciudad'] ?? '') ?>, <?= htmlspecialchars($cuenta->fechaEmision) ?></dd></div>
                    <?php if (!empty($cuenta->fechaVencimiento)): ?>
                        <div><dt>Vence:</dt><dd><?= htmlspecialchars($cuenta->fechaVencimiento) ?></dd></div>
                    <?php endif; ?>
                    <div><dt>Estado:</dt><dd><?= htmlspecialchars($cuenta->estado) ?></dd></div>
                </dl>
            </div>
        </div>
        ...
        <div class="concepto">
            ...
            <p style="font-size:11px;color:#475569;line-height:1.45;white-space:pre-wrap;">
                <?= nl2br(htmlspecialchars($cuenta->concepto)) ?>
            </p>
        </div>
        ...
        <div class="firma">
            ...
            <p class="muted" style="font-size:10px;margin-top:4px;">Se firma en <?= htmlspecialchars($datosEmisor['Ciudad'] ?? '________') ?>, a los <?= htmlspecialchars($diaTexto) ?> días del mes de <?= htmlspecialchars($mesNombre) ?> de <?= htmlspecialchars($anioTexto) ?>.</p>
        </div>
