...
<body class="min-h-screen font-sans text-slate-700 text-[13px] leading-relaxed">
    ...
    <div class="w-full px-4 pb-8 flex justify-center">
        <div id="area-cotizacion" class="w-full max-w-[8.5in] bg-white border border-slate-200 shadow-xl rounded-[32px] px-12 py-12 space-y-10">
            <header class="grid gap-10 md:grid-cols-2">
                ...
                <div class="md:text-right space-y-3">
                    <p class="text-xs uppercase tracking-[0.4em] text-slate-400">Cotización</p>
                    <p class="text-4xl font-black text-slate-900 tracking-[0.35em]">Nº <?= htmlspecialchars($cotizacion->numeroCotizacion) ?></p>
                    <dl class="text-sm text-slate-600 space-y-1">
                        <div><dt class="font-semibold inline">Ciudad y fecha:</dt> <dd class="inline"><?= htmlspecialchars(($datosEmisor['Ciudad'] ?? '')) ?>, <?= htmlspecialchars($cotizacion->fechaEmision) ?></dd></div>
                        <?php if (!empty($cotizacion->fechaVencimiento)): ?>
                            <div><dt class="font-semibold inline">Válida hasta:</dt> <dd class="inline"><?= htmlspecialchars($cotizacion->fechaVencimiento) ?></dd></div>
                        <?php endif; ?>
                        <div><dt class="font-semibold inline">Estado:</dt> <dd class="inline"><?= htmlspecialchars($cotizacion->estado) ?></dd></div>
                    </dl>
                </div>
            </header>
